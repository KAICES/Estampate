...
public class VentasServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            for(Usuario u:misV){      
            for( Camisetas c:misC) {
            for(Estampas es:misES) {
                
                for(FacturaVenta f:u.getFacturaVentas()){
                    for(FacturaVenta f1:c.getFacturaVentas()){
                        for(FacturaVenta f2:es.getFacturaVentas()){
                                  
              out.println("<tr><td>"+u.getNombre()+"</td><td></td><td> $ "+f1.getValorArtista()+" </td><td>"+f.getValorTotal()+"</td><td>"+f2.getEstampas()+"</td></tr>");     
